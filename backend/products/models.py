...
class Delivery(models.Model):
    ...
    title = models.CharField(
        ...
        help_text=(
            "Для checkout (курьер): «км» — тариф за 1 км; «базов»/«подъезд»/«выезд» — фикс к сумме; "
            "«минимальн» — нижняя граница доставки; «макс»/«потолок» — верхняя; "
            "«бесплатн» + «доставк»/«курьер»/«заказ» — порог суммы заказа для бесплатной доставки."
        ),
    )
    value_number = models.DecimalField(
        ...
        help_text=(
            "BYN. Км — за километр (начисление по начатым км); базов/подъезд — фикс; "
            "минимальн/макс — ограничения итога; бесплатн — мин. сумма заказа для 0 BYN доставки."
        ),
    )
